...
15. [~] **Google Search Console — оказа се, че property изобщо няма.** Проверени 3 профила (`dimitrovlacho@`, `lachezar0dimitrov@`, третия) — нула property-та, нула чакащи верификации. Значи няма паднала DNS верификация; никога не е имало. Остава да се провери `carnivalforyou@gmail.com` (бизнес пощата) и евентуално property под чужд профил (фирмата, правила стария сайт).
    - Следствие: manual actions **не могат да се видят без property**, а ръчна мярка може да съществува срещу домейна и без такова. Предвид историята със спама, property-то трябва да се създаде **преди** да поканим Google да обхожда 1256 нови URL-а.
    - Препоръка: **Domain property** (не URL-prefix) — покрива всички поддомейни и http/https, верифицира се с TXT в Cloudflare зоната. ⚠️ Записът трябва да е НОВ, отделен TXT на apex-а — да НЕ се слива със SPF записа (правилото "точно един" важи само за SPF).
    - Базова стойност отпреди миграцията: `site:carnivalforyou.com` дава **~4890 резултата** — старият сайт е сериозно индексиран, значи има реален SEO капитал за пренасяне. (Публичното число е силно закръглено от Google и не е съпоставимо с Pages отчета.)
...
6. [ ] **Вкарай Google Analytics** (решено 2026-09-08 да се направи; може непосредствено преди или скоро след cutover, не блокира миграцията).
   - ⚠️ **Задължително заедно с това:** в момента cookie банерът е чисто декоративен — [src/components/CookieConsent.tsx](src/components/CookieConsent.tsx) само записва избора в localStorage (`cfy-cookie-consent`) и се скрива; "Отказвам" не спира нищо, защото няма какво да спре. При вкарване на GA скриптът трябва да се зарежда **само** при `accepted`, иначе банерът остава подвеждащ и съгласието — недействително.
   - Контекст от одита 2026-09-08: сайтът днес не слага **никакви** собствени бисквитки (`document.cookie` не се ползва никъде), няма нито един тракер, а трите localStorage ключа (език, splash-seen, самото съгласие) + Supabase admin сесията са строго функционални. Единственият чужд източник на бисквитки в момента е Google Maps iframe-ът на Контакти, който се зарежда безусловно — струва си да мине на click-to-load, когато се пипа съгласието за GA.
