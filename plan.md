...
15. [ ] 🟡 **Голяма, отделна тема — английската версия на сайта е невидима за Google.** Виж [[project_carnivalforyou_english_seo_gap]] за пълния анализ. Резюме: старата база е била напълно двуезична с отделни `?lang=en` URL-и (вероятно обяснява половината от baseline-а ~4890 site: резултата); новият сайт пази езика само в `localStorage`, няма URL представяне, `hreflang` е невъзможен в текущата форма, и старите redirect функции изцяло игнорират `lang` параметъра — всеки индексиран стар EN адрес днес отвежда към българска страница. 1881 от активните продукти вече имат готов, непреведен от нас EN текст в legacy export-а, ако решим да го ползваме. Не е започнато — планирано за по-късно, по избор на потребителя.
