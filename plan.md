...
6. [ ] Довърши metadata/title/description консистентност (CLAUDE.md §6) от старото CSV копие (`carnival_seo_page_meta.csv` `tt-*` записите за категории вече съвпадат с `tid` 2,3,4,17,15) — код в новия сайт, безопасно да се пише сега.
7. [ ] Провери/довърши sitemap.xml за новия сайт — пак код/build артефакт, безопасно сега.
