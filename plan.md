...
- [x] **Soft-404** — частично затворено 2026-09-17 (commit `8dc4cb0`), с
  евтиния/безопасен вариант вместо пълен whitelist: blocklist на познати
  bot-probe пътища (`/wp-admin`, `/xmlrpc.php`, `/.env`, `/wp-login.php`,
  `/wp-content`, `/.git`, `/wp-config.php`, `/config.php`,
  `/administrator`, `/phpmyadmin`) вече връща истинско 404, всичко останало
  си минава през същия `/* /index.html 200` catch-all както преди —
  съзнателно тесен обхват, не пълен route whitelist (риск от 404 на реална
  страница при грешка в списъка). **Важна бележка:** `public/_redirects` НЕ
  поддържа 404 статус код (само 200 и 3xx се прилагат — първият опит с
  правило там тихо не правеше нищо ~20 мин, докато не се провери с
  Cloudflare docs). Логиката живее в `functions/_middleware.js`, който вече
  прихваща всяка заявка. Валидирано на живо: всички blocklist пътища → 404,
  случаен непознат адрес → пак 200 (catch-all непокътнат), всички реални
  маршрути (/, /products, /product-detail/*, /en, /about, sitemap.xml,
  robots.txt, /products.php legacy redirect, /admin) работят нормално.
  Пълен route whitelist остава неопитан — ако някога GSC покаже реален обем
  soft-404 от адреси извън тази blocklist, тогава си струва отделен проход.
