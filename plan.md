...
13. [~] 🔴 **Вкарай Google Analytics** (решено 2026-09-08 да се направи; не блокира миграцията, но е приоритетно).
    ...
    - ⚠️ **Честно казано, все още не напълно потвърдено.** Тествах локално (production build + Playwright browser) същия сценарий — и дори с каноничния код не видях `/g/collect` заявка да излиза от самата библиотека (нито автоматично след `config`, нито при ръчно повикан `gtag('event',...)`), макар `gtag.js` да сваля истинско, ID-специфично съдържание (519 KB, 20 споменавания на `G-9LQ9WXM8YT`, не заглушка) и `window.google_tag_manager['G-9LQ9WXM8YT']` да се създава (`dataLayer`/`callback`/`bootstrap`), но никога да не продължи напред. Ръчен `fetch()` до `google-analytics.com/g/collect` от същата страница мина веднага (204) — мрежата не е блокирана, само вътрешната логика на библиотеката не тръгва да праща.
      - И extension-ът по-рано, и аз сега през различен автоматизиран browser виждаме идентичен модел — възможно двете агентски browser среди пясъчнstriктират/заглушават реалното изпращане на tracking hit-ове по начин, различен от обикновено мрежово блокиране, а не непременно остатъчен бъг в кода — но не е доказано на 100%.
      - **Задължителна следваща стъпка, преди да се счита за напълно решено:** провери от истински, неавтоматизиран браузър (лично устройство, обикновена мрежа, НЕ през Claude тул) на живия сайт — DevTools → Network → `g/collect` заявка след "Приемам", и/или GA4 Realtime. Ако пак е 0 — тогава има реален проблем за по-нататъшна диагностика (Google Tag Assistant extension дава по-детайлна диагностика от голия Network tab).
