...
12. [x] ✅ **24-часов мониторинг направен 2026-09-08 (Chrome extension), период 7 сеп 19:00 UTC → 8 сеп 16:20 UTC (~21ч). Резултат: миграцията сама по себе си е чиста.**
    ...
    - **Намерени DNS остатъци от старото хостване — почистени същия ден 2026-09-08 (Chrome extension), веднага след потвърждение, че нищо не ги ползва:**
      - ✅ **Изтрити изцяло** (5 CNAME записа, потвърдено вече NXDOMAIN): `mail.`, `pop.`, `pop3.`, `smtp.`, `imap.carnivalforyou.com`. Причина: DNS-only CNAME верига през `carnivalforyou.com`, което вече е proxied към Pages → 170×522 (преди миграцията същата верига стигаше до jump.bg IP-то, обслужвало и пощата). Потвърдено с потребителя: цялата поща минава през Gmail, никой не конфигурира реален mail клиент срещу тези hostname-и — безопасно за изтриване. `pop3.` не беше в първоначалния списък, хванат допълнително от extension-а при прегледа на цялата зона, добавен по същата логика.
      - ✅ **Сменени на "DNS only"** (спряно проксиране, записите остават): `test.`, `new.carnivalforyou.com`. Причина: proxied към старото jump.bg IP (185.199.38.18), поемаха 819×403 сканиращ трафик (не реални посетители) + Cloudflare предупреждаваше за частично изложен origin IP. Потвърдено: нямат активна употреба; `test.carnivalforyou.com` проверен визуално да продължава да зарежда директно от origin-а (нормално, само вече без Cloudflare прокси междинно).
      - **Непокътнато, проверено** — и двата пъти преди и след промяната: и трите MX записа, двата DKIM TXT записа, SPF TXT на apex-а, `google-site-verification` записът.
      - Броят записи в зоната: 39 → 34.
      - ℹ️ Apex/www CNAME towards `carnivalforyoupoc3.pages.dev` (не poc4) — вече потвърдено правилно при go-live, не нова находка, не пипнато.
      - Дребно, все още отворено: `/images/favicon.ico` 404 — браузърски default probe, ниска важност, не спешно.
...
14. [x] **SEO подобрения след go-live одит, 2026-09-08 (`4195060`, merge-нато в `main`).** Направено в `dev/seo-improvements`, валидирано срещу production build + Playwright (screenshot потвърди дизайнът е непроменен) преди merge:
    - Заглавна структура: началната имаше 3 конкурентни `<h1>` (банер слайдове), About/Services/News/Contacts/Terms нямаха нито един. Банерите → `h2`; `SectionHeading` взима опционален `as="h1"`; страници без видимо заглавие в дизайна (Начало/Новини/Услуги) получиха `sr-only` H1 — стилът не се променя.
    - `BreadcrumbList` schema на продуктовите страници (Начало → Категория → Продукт), плюс `brand`/`seller` (реферира `#business` от `index.html`) в Product schema. Breadcrumb-ите прескачат скритите категории.
    - Sitemap `lastmod` вече не е "днес" за статични/категорийни URL-и при всяко извикване — това обезценяваше датите на продуктовите URL-и, единствените с реална дата.
    - Не пипнато нарочно: server-side JSON-LD в `functions/product-detail/[id].js` (рисково без `wrangler` за локален тест — тази функция вече веднъж се е чупила по начин невидим локално), code-splitting на bundle-а (без данни за реален LCP проблем), скритите категории (бизнес решение, не техническо).

15. [x] ✅ **Английска версия на сайта — `/en` вече е реален, crawlable URL, изградено и валидирано 2026-09-08 в `dev/en-locale-routes` (3 commit-а: `666d94d`, `db3fbd0`, `94c636d`), още НЕ merge-нато в `main`.** Виж [[project_carnivalforyou_english_seo_gap]] за пълния анализ/решение. Четири фази:
    - **Фаза 1 (routing):** `router.tsx` парсва `/en` префикс и го прави source of truth за езика; `switchLanguage()` пренасочва към текущата страница на другия език (не към начало); `I18nProvider` вече чете езика от route-а за всички публични страници, `/admin` пази старото си независимо поведение непроменено (нищо там не вика `setLang` и преди). Валидирано с Playwright през всичките 8 маршрута × 2 езика + browser back/forward + screenshot diff (пиксел идентичен дизайн).
    - **Фаза 2 (SEO сигнали):** `useSEO.ts` автоматично генерира `hreflang` двойка (bg/en/x-default) от текущия canonical; `sitemap.xml.js` вече emit-ва и двата езика с `xhtml:link` анотации, продуктите само когато имат реално EN описание (self-completing — виж Фаза 4); статичен hreflang чифт в `index.html`. 🔴 **Съзнателно НЕ пипнат** `functions/product-detail/[id].js` (живият BG endpoint, 1230+ реални URL-а, вече веднъж се е чупил невидимо локално) — новият `/en/product-detail/:id` е отделен файл (`functions/en/product-detail/[id].js`), споделя само чисти helper функции през `functions/_lib/productMeta.js`, никога orchestration логиката. **Все още изисква проверка срещу реален deploy** (Preview Deployments са изключени за проекта) — практически означава тест на живия URL веднага след merge, по същия начин както оригиналният BG бъг е бил хванат навремето.
    - По пътя хванат и оправен реален бъг: `ProductsPage.tsx`-ката canonical override за категорийни листвания беше hardcoded без `/en` префикс — `/en/products?category=2` се self-канонизираше към българския адрес. Хванато и оправено с Playwright преди commit.
    - **Фаза 3 (legacy redirects):** `products.php.js`/`t_prod.php.js`/`holds.php.js`/`about.php.js` вече четат `&lang=en` от старите URL-и и пренасочват към `/en/...` еквивалента. Тествано реално срещу живия Supabase — всички комбинации коректни, включително стария `cntid=18` special case и tid=999 fallback.
    - **Фаза 4 (съдържание):** 27-те активни, видими продукта без `description_en` (не 412 — стеснено до реално видимия каталог) вече имат написани английски описания, директно в Supabase, в стила на съществуващите (кратки, изброяващи съдържание/размери) — НЕ от стария CSV export, по изричното решение на потребителя. Потвърдено на живо: sitemap.xml вече emit-ва всичките 1230 EN продуктови URL-а (преди: 1203), `/en/product-detail/562` вече показва реалния текст + hreflang тагове — без redeploy, точно както е проектирано.
    - ✅ **Merge-нато в `main` 2026-09-08 (`4575b74`), тествано директно срещу живия carnivalforyou.com веднага след деплоя** — точно както планът изискваше. Хванат и оправен реален бъг при самия live тест: статичните hreflang tag-ове от `index.html` (сочещи към началото) изтичаха непроменени в продуктовите страници server-side, а новият EN endpoint добавяше свой чифт отгоре без да маха стария — 6 конфликтни tag-а вместо 3. Оправено с нов `AlternateRemover`/`ElementRemover` handler и в двата product-detail файла (`60b7129`) — минимална, чисто добавъчна промяна дори в иначе недокоснатия BG файл (само маха остарели, не пипа title/OG/canonical логиката). Потвърдено на живо след втория deploy: EN продукт → точно 3 коректни hreflang tag-а; BG продукт → 0 (правилно, разчита на клиентския JS); legacy redirect с `&lang=en` → коректен `/en/...` адрес; sitemap.xml → 200, 2512 URL записа.
    - **Остава:** GSC submit на новия sitemap + евентуално `/en` verification (Chrome extension).
