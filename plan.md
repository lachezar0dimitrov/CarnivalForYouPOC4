...
13. [ ] 🔴 **Вкарай Google Analytics** (решено 2026-09-08 да се направи; не блокира миграцията, но е приоритетно).
    - ⚠️ **Задължително заедно с това:** cookie банерът в момента е чисто декоративен — [src/components/CookieConsent.tsx](src/components/CookieConsent.tsx) само записва избора в localStorage (`cfy-cookie-consent`) и се скрива; "Отказвам" не спира нищо, защото няма какво да спре. При вкарване на GA скриптът трябва да се зарежда **само** при `accepted`, иначе банерът остава подвеждащ и съгласието — недействително.
