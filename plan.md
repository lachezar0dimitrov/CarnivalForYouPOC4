...
13. [~] 🔴 **Вкарай Google Analytics** (решено 2026-09-08 да се направи; не блокира миграцията, но е приоритетно).
    ...
    - [ ] **Остава, изисква Chrome Extension:** добави същия `VITE_GA_MEASUREMENT_ID=G-9LQ9WXM8YT` и в Cloudflare Pages → Settings → Environment variables → **Production** (не само Preview) — същата грешка като по-рано с Supabase env vars е лесна тук: ако липсва само в Production, build минава чисто, но GA мълчаливо никога не тръгва в прод. След добавяне: нужен нов deploy (retrigger или следващ push), после провери в браузър (DevTools → Network) на живия сайт, след като приемеш cookie банера, че `gtag/js?id=G-9LQ9WXM8YT` реално се зарежда.
    - [ ] Кодовите промени (analytics.ts + гейтването в CookieConsent + click-to-load картата) все още са **само локални, не commit-нати/push-нати** — трябва push към `main`, за да стигнат до Cloudflare Pages изобщо.
