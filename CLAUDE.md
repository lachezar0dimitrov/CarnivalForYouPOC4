...
## 10. Invoice Import Tool (Rubies UK) — 2026-09-13

Two-part tool for turning a new stock invoice into ready-to-review catalog rows, replacing fully-manual product entry for new deliveries.

- **Launching it**: double-click `import-invoice.cmd` (or the `Import Invoice.lnk` shortcut some machines need — see below) instead of typing the npm command; a native Windows "Open File" dialog ([scripts/import-invoice/pickFile.ts](scripts/import-invoice/pickFile.ts)) picks the PDF, no CLI args required. `.lnk` shortcuts are gitignored (they bake in an absolute machine path) — `import-invoice.cmd` itself is tracked and portable.
- **Part 1 — local script**: `npm run import:invoice` ([scripts/import-invoice/](scripts/import-invoice/)). Parses the invoice PDF (a custom row-aware `pdf-parse` `pagerender` reconstructs table rows by Y/X position — the default text extraction scrambles this invoice's columns, verified against a real Rubies invoice before writing the row regex), matches each item code against rubiesuk.com (direct `/products/<sku>` guess, else `/search?q=`, trying several size-suffix-stripped candidates), scrapes the product's schema.org JSON-LD block (name/description/image/sku — far more complete than og: tags or theme-specific DOM selectors, and the sku is cross-checked against the invoice code as a confidence signal), downloads the photo + generates a thumbnail (`sharp`), translates name/description EN→BG in one batched call through the **`claude` CLI** ([scripts/import-invoice/translate.ts](scripts/import-invoice/translate.ts) — `spawn`s the local `claude -p --output-format json --json-schema ...` binary, run from the script's own directory with tools disallowed to cut the fixed per-call token overhead; deliberately *not* `@anthropic-ai/sdk` — the client is on a Pro/Max plan and didn't want a separately-metered API key, and the CLI call draws on that plan's included usage instead. The prompt explicitly tells it to rewrite Rubies' B2B wholesale/retailer marketing copy into a short customer-facing rental description — translating it verbatim reads bizarrely on a consumer rental site), and writes an `.xlsx` review sheet (`exceljs`, embedded thumbnails, every row a fixed 80px height with word wrap on) to `imports/<date>/` (gitignored — working data, not source), then auto-opens it (`start` via `child_process.exec`). **First column is `catalog_number`** — seeded with the manufacturer's invoice code, but there is no separate "item code" field kept anywhere: the admin overwrites this cell in place with the shop's real physical tag number before upload (per explicit client instruction — one number, not two; `images/original/<file>.jpg` filenames still key off the original invoice code, so only edit this column, never `image_file`). **Categories are one checkbox column per active category** (header = the category's Bulgarian name, "x" = checked), not a single dropdown — a product can genuinely belong to several categories at once, matching the DB's `category_ids` array. The keyword-based pre-check ([scripts/import-invoice/categories.ts](scripts/import-invoice/categories.ts), [category.ts](scripts/import-invoice/category.ts)) fetches the live `categories` table (not hardcoded IDs, so it stays correct as categories are added/renamed) and ticks a demographic box plus, when detected, one theme box (Halloween/Pirates/Licensed & Movie Characters/Historical/Retro/Fairy Tale/Novelty — checked in that specificity order since most items would otherwise just get "Halloween"). A wrong or missing guess just means the admin ticks a different box before upload. The identifying columns are frozen so they stay in view while scrolling right through the (~20) category columns. Downloaded originals are real manufacturer-resolution photos (verified: 1500×2000px, 60–200KB) — the small 240×320 thumbnails live in a separate `images/thumb/` folder used only for the Excel preview, never uploaded.
- **Part 2 — admin loader**: Admin → Импорт tab ([src/pages/admin/ImportInvoicePanel.tsx](src/pages/admin/ImportInvoicePanel.tsx)). Admin uploads the reviewed `.xlsx` plus the `images/original` folder, "Провери" validates every row (`catalog_number` present + not a duplicate in the DB or in the sheet, required fields, at least one checked category column resolved against the live `categories` table, a matching image file) and refuses to upload anything if any row has an error — only a fully clean sheet enables "Качи в сайта", which uploads each image via the existing `r2-media` function ([src/lib/r2.ts](src/lib/r2.ts)) and inserts the product (all checked categories → `category_ids`, first one → `category_id`, `catalog_number` → `old_catalog_number`) with `is_active: true`, `is_new: true`, `new_since: now()`.
- **Auto-expiring "new" badge**: `products.new_since` (migration `20260913130100_add_products_new_since.sql`) is set only by this loader. `fetchNewProducts()` in [src/lib/products.ts](src/lib/products.ts) hides a product from the homepage ribbon once `new_since` is more than 6 months old — computed at query time, no cron job. `new_since` stays `NULL` for every product flagged "new" the old way (the `ProductForm` checkbox), so existing manually-flagged products are unaffected and never expire.
- The `xlsx` (SheetJS) dependency used by the admin panel to parse the sheet client-side is dynamically `import()`-ed inside `ImportInvoicePanel.tsx`, not top-level — `AdminPage` is statically imported in `App.tsx`, so a top-level import would have shipped ~430KB to every public site visitor. Verified via `npm run build`: it now lands in its own chunk, loaded only when the Import tab is used.
- Full end-to-end generation (site-matching, scraping, thumbnails, `claude` CLI translation, checkbox categories) was verified against the real Invoice 0000087969 — 54/56 items matched, both translation and checkbox categories confirmed populated correctly. The admin loader's "Качи в сайта" step (actual R2 upload + DB insert) has not yet been exercised against a real sheet — verify that next before trusting a real import. This whole feature sat **uncommitted** in the working tree for a day (the Import tab was invisible on the deployed site as a result) until committed/pushed on 2026-09-14 — if the tab is ever missing again, check `git log -- src/pages/admin/ImportInvoicePanel.tsx` first before re-debugging the code itself.
